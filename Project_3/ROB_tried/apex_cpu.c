    ...
    if(cpu->LSQ.has_insn)
    {
        ...
        switch(cpu->LSQ.opcode)
        {
            ...
            case OPCODE_LOAD:
            {
                ...
                cpu->regs[cpu->LSQ.rd] = cpu->LSQ.result_buffer;
        ...
        ROBQ[cpu->LSQ.robIndex].ins = cpu->LSQ;
        ROBQ[cpu->LSQ.robIndex].commited = true;
...
APEX_ROB(APEX_CPU *cpu)
{
    if(cpu->ROB.has_insn)
    {
        if(ROBQ[0].commited == TRUE)
        {
            //cpu->ROB.robIndex = 0;
            cpu->writeback = ROBQ[0].ins;

            
            //cpu->regs[ROBQ[0].ins.rd] = ROBQ[0].ins.result_buffer;


            printf("\n\nCommited : ROB[%d] : %s R[%d] R[%d] R[%d] #%d : result_buffer = %d\n\n", 0, ROBQ[0].ins.opcode_str, ROBQ[0].ins.rd, ROBQ[0].ins.rs1, ROBQ[0].ins.rs2, ROBQ[0].ins.imm, ROBQ[0].ins.result_buffer);
            ROBQ[0] = ROBQ[1];
            ROBQ[1] = ROBQ[2];
            ROBQ[2] = ROBQ[3];
            ROBQ[3] = ROBQ[4];
        ...
        ROBQ[currentIndex].ins = cpu->ROB;
        cpu->ROB.robIndex = currentIndex;

        if(cpu->ROB.opcode == OPCODE_STORE || cpu->ROB.opcode == OPCODE_LOAD || cpu->ROB.opcode == OPCODE_LDR || cpu->ROB.opcode == OPCODE_STR)
        {
            cpu->LSQ.robIndex = currentIndex;
            ROBQ[currentIndex].commited = TRUE;
        }
        else
        cpu->IQ.robIndex = currentIndex;
        ...
        for(int i=0;i<8;i++)
        {
            printf("ROB[%d] : %s R[%d] R[%d] R[%d] #%d\n", i, ROBQ[i].ins.opcode_str, ROBQ[i].ins.rd, ROBQ[i].ins.rs1, ROBQ[i].ins.rs2, ROBQ[i].ins.imm);
        }
        printf("\n\n");
...
APEX_ifu(APEX_CPU *cpu)
{
    ...
    if (cpu->ifu.has_insn)
    {
        ...
        // ROBQ[cpu->ifu.robIndex].ins.result_buffer = cpu->ifu.result_buffer;
        // cpu->tempStage = cpu->ifu;

        
        //ROBQ[cpu->ifu.robIndex].commited = TRUE;

        //cpu->ifu.

        //printf("\n\ncpu->ifu.robIndex : %d\n\n", cpu->ifu.robIndex);

        ROBQ[cpu->ifu.robIndex].ins = cpu->ifu;
        ROBQ[cpu->ifu.robIndex].commited = TRUE;
        //cpu->ifu.robIndex = 0;
        //cpu->tempStage = cpu->ifu;
...
static void
APEX_tempStage(APEX_CPU *cpu)
{
    if (cpu->tempStage.has_insn)
    {
        //ROBQ[cpu->tempStage.robIndex].ins = cpu->ifu;
        ROBQ[cpu->tempStage.robIndex].commited = TRUE;
...
APEX_mul4(APEX_CPU *cpu)
{
    ...
    if (cpu->mul4.has_insn)
    {
        ...
        ROBQ[cpu->mul4.robIndex].ins = cpu->mul4;
        ROBQ[cpu->mul4.robIndex].commited = TRUE;
        //cpu->mul4.robIndex = 0;

        // cpu->writeback = cpu->mul4;
...
APEX_lsfu(APEX_CPU *cpu)
{
    ...
    if (cpu->lsfu.has_insn && cpu->lsfu.isStalled == FALSE)
    {
        ...
        ROBQ[cpu->lsfu.robIndex].ins = cpu->lsfu;
        ROBQ[cpu->lsfu.robIndex].commited = TRUE;
        //cpu->lsfu.robIndex = 0;
...
void print_register_state(APEX_CPU* cpu) 
{
  ...
  int size = (int) (sizeof(cpu->regs)/sizeof(cpu->regs[0]));

  for(int i = 0; i < size; ++i)
  {
    printf("|      REG[%d]       |       Value = %d        |         Status = %s        \n", i, cpu->regs[i], (!inUseArray[i] ? "VALID" : "INVALID"));
  }
